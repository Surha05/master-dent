...
if ($connection) {
	mysqli_query($connection, "UPDATE `advantage` SET `name` = '$name', `description` = '$description'  WHERE `advantage`.`id` = '$id';");
}
if ($photo) {
	mysqli_query($connection, "UPDATE `advantage` SET `photo` = '$photo'  WHERE `advantage`.`id` = '$id';");
}
